...
ReadableSensors::ReadStatus POSCanSensor::receiveData()
{
  ...
  if (canReceive(receiveBuffer) == CanSensor::CanReadStatus::CAN_READ_SUCCESS)
  {
    ...
    posData.x          = (receiveBuffer[0] << 8) | receiveBuffer[1];
    posData.y          = (receiveBuffer[2] << 8) | receiveBuffer[3];
    posData.theta      = (receiveBuffer[4] << 8) | receiveBuffer[5];
    posData.servoTheta = (receiveBuffer[6] << 8) | receiveBuffer[7];
