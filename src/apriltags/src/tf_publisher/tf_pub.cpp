...
void detectionCallback(const apriltags::AprilTagDetectionsConstPtr& msg)
{
  ...
  for (int ctr = 0; ctr < msg->detections.size(); ctr++)
  {
    ...
    {
      tf::Quaternion q(msg->detections[ctr].pose.orientation.x, msg->detections[ctr].pose.orientation.y,
                       msg->detections[ctr].pose.orientation.z, msg->detections[ctr].pose.orientation.w);
      transform.setOrigin(tf::Vector3(msg->detections[ctr].pose.position.x, msg->detections[ctr].pose.position.y,
                                      msg->detections[ctr].pose.position.z));
