...
namespace low_pass_namespace
{
  ...
  void LowPassLayer::onInitialize ()
  {
    ...
    dynamic_reconfigure::Server<costmap_2d::GenericPluginConfig>::CallbackType cb = boost::bind (&LowPassLayer::reconfigureCB, this, _1, _2);
    dsrv->setCallback(cb);
  }
  ...
  void LowPassLayer::updateBounds(double robot_x, double robot_y, double robot_yaw, double* min_x,
  ...
  void LowPassLayer::updateCosts(costmap_2d::Costmap2D& master_grid, int min_i, int min_j, int max_i,
                              int max_j)
  {
    if (!enabled_)
      return;
