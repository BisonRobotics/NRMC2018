...
WaypointController::Status WaypointController::update(pose robotPose, float dt)
{
  ...
  {
    if (!doingManeuver) //maneuver is completed or one has not yet been started, need to calculate end point and base wheel speeds
    {
        ...
        maneuverEnd = endOfManeuver(navigationQueue.at(0).initialPose, currMan); //the end pose is the extension from the initial pose
        else
    ...
    Status returnStatus;
    if(APPROX(DIST(robotPose.x, robotPose.y, theCPP.x, theCPP.y), 0, BADLINE)) returnStatus = Status::ALLGOOD;
	else returnStatus = Status::ALLBAD;
